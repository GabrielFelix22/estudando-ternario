...
function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-gray-900 text-white">
      <div className="flex flex-col gap-4 border border-gray-700 p-6 rounded-2xl shadow-md shadow-blue-500/80">
      ...
      <ul className="flex flex-col gap-2 mt-4 text-lg">
        {funcionarios.map((funcionario) => (
          <li key={funcionario.nome} className="flex justify-between w-full gap-1">
            {`${funcionario.nome}:`}
            <span
              className={
                funcionario.tipo === "CLT" ? "text-green-500" : "text-purple-500"
              }
            >
              {funcionario.tipo}
            </span>
        ...
        <button type="button" onClick={() => setSeguindo(!seguindo)} className="w-full mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded cursor-pointer">
